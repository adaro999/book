 ...
 <!-- <header class="bg-dark py-5" id="main-header">
    <div class="container h-100 d-flex align-items-center justify-content-center w-100">
        ...
        <h1 class="display-4 fw-bolder">Available Electricians</h1>
    ...
    <div class="container px-4 px-lg-5 mt-5 card rounded-0 card-outline card-purple shadow">
        <div class="row">
            ...
            <hr>
                ...
                <div class="row gx-4 gx-lg-5 row-cols-1 row-cols-sm-1 row-cols-md-2 row-cols-xl-3" id="electrician_user">
                    <?php 
                    $cabs = $conn->query("SELECT c.*, cc.name as category FROM `electrician_user` c inner join category_list cc on c.category_id = cc.id where c.delete_flag = 0 and c.id not in (SELECT cab_id FROM `booking_list` where `status` in (0,1,2)) order by c.`electricianSer_ID`");
                        ...
                        <div class="callout callout-primary border-success rounded-0">
                            <dl>
                                ...
                                <dd class="truncate-3 text-muted lh-1">
                                    ...
                                    <small><?php echo $row['expertise_text'] ?></small>
                                </dd>
                            </dl>
                        </div>
                    </a>
...
<script>
    $(function(){
        $('#search').on('input',function(){
            ...
            $('#electrician_user .item').each(function(){
            ...
            if( $('#electrician_user .item:visible').length > 0){
                $('#noResult').hide('slow')
            }else{
                $('#noResult').show('slow')
            }
        })
        $('#electrician_user .item').hover(function(){
            $(this).find('.callout').addClass('shadow')
        })
        $('#electrician_user .book_cab').click(function(){
            if("<?= $_settings->userdata('id') && $_settings->userdata('login_type') == 2 ?>" == 1)
                uni_modal("Book Electrician - "+$(this).attr('data-bodyno'),"booking.php?cid="+$(this).attr('data-id'),'mid-large');
            else
