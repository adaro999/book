...
Class Master extends DBConnection {
	...
	function save_cab(){
		...
		if(empty($_POST['id'])){
			...
			$code = sprintf("%'.05d",1);
			while(true){
				$check = $this->conn->query("SELECT * FROM `electrician_user` where electricianSer_ID = '{$prefix}{$code}'")->num_rows;
				if($check > 0){
			...
			$_POST['electricianSer_ID'] = $prefix.$code;
		}
		...
		if(isset($user_ID_no)){
			$check = $this->conn->query("SELECT * FROM `electrician_user` where `user_ID_no` = '{$user_ID_no}' ".(!empty($id) ? " and id != {$id} " : "")." ")->num_rows;
			if($this->capture_err())
				return $this->capture_err();
			if($check > 0){
				$resp['status'] = 'failed';
				$resp['msg'] = " Electrician already exist.";
				return json_encode($resp);
				exit;
			}
		}
		if(isset($body_no)){
			$check = $this->conn->query("SELECT * FROM `electrician_user` where `body_no` = '{$body_no}' ".(!empty($id) ? " and id != {$id} " : "")." ")->num_rows;
			if($this->capture_err())
				return $this->capture_err();
			if($check > 0){
				$resp['status'] = 'failed';
				$resp['msg'] = " Account number already exist.";
				return json_encode($resp);
				exit;
			}
		}
		if(isset($oldpassword)){
			$cur_pass = $this->conn->query("SELECT `password` from `electrician_user` where id = '{$this->settings->userdata('id')}'")->fetch_array()[0];
		...
		if(empty($id)){
			$sql = "INSERT INTO `electrician_user` set {$data} ";
			$save = $this->conn->query($sql);
		}else{
			$sql = "UPDATE `electrician_user` set {$data} where id = '{$id}' ";
			$save = $this->conn->query($sql);
		}
		if($save){
			$resp['status'] = 'success';
			...
			$resp['id'] = $cid ;
			if(empty($id))
				$resp['msg'] = " New Electrician successfully saved.";
			else
				$resp['msg'] = " Electrician info successfully updated.";
				...
				if(isset($_FILES['img']) && $_FILES['img']['tmp_name'] != ''){
					...
					if(isset($uploaded_img)){
						$this->conn->query("UPDATE electrician_user set `image_path` = CONCAT('{$fname}','?v=',unix_timestamp(CURRENT_TIMESTAMP)) where id = '{$cid}' ");
	...
	function delete_cab(){
		extract($_POST);
		$del = $this->conn->query("UPDATE `electrician_user` set `delete_flag` = 1  where id = '{$id}'");
		if($del){
			$resp['status'] = 'success';
	...
	function save_booking(){
		...
		if($save){
			$resp['status'] = 'success';
			if(empty($id))
				$this->settings->set_flashdata('success'," Request has been booked successfully.");
			else
