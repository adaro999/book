...
Class Users extends DBConnection {
	...
	public function save_client(){
		...
		if(isset($_POST['oldpassword'])){
			if($this->settings->userdata('id') > 0 && $this->settings->userdata('login_type') == 2){
				$get = $this->conn->query("SELECT * FROM `homeowner_user` where id = '{$this->settings->userdata('id')}'");
				$res = $get->fetch_array();
		...
		$check = $this->conn->query("SELECT * FROM `homeowner_user` where email = '{$email}' and delete_flag ='0' ".(is_numeric($id) && $id > 0 ? " and id != '{$id}'" : "")." ")->num_rows;
		if($check > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = ' Email already exists in the database.';
		}else{
			if(empty($id)){
				$sql = "INSERT INTO `homeowner_user` set $data";
			}else{
				$sql = "UPDATE `homeowner_user` set $data where id = '{$id}'";
			}
			...
			if($save){
				...
				if(isset($_FILES['img']) && $_FILES['img']['tmp_name'] != ''){
					...
					if(isset($uploaded_img)){
						$this->conn->query("UPDATE homeowner_user set `image_path` = CONCAT('{$fname}','?v=',unix_timestamp(CURRENT_TIMESTAMP)) where id = '{$uid}' ");
	...
	function delete_client(){
		extract($_POST);
		$del = $this->conn->query("UPDATE `homeowner_user` set delete_flag = 1 where id='{$id}'");
		if($del){
			$resp['status'] = 'success';
